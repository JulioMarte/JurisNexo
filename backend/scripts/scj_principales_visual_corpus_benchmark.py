...
def run_model() -> int:
    ...
    store = build_s3_object_store(); provider = _provider(); records: list[VisualCorpusRecord] = []; details: list[dict[str, Any]] = []; running_cost = 0.0
    for ordinal, sample in enumerate(samples, start=1):
        ...
        try:
            ...
            health = assess_reference_text_health(reference); started = time.perf_counter()
            ...
            latency_ms = int((time.perf_counter() - started) * 1000); transcription = str(result.value.get("transcription") or "")
            score = score_text_fidelity(expected_text=reference, candidate_text=transcription); expected_critical = sum(item.expected for item in score.critical.values()); matched_critical = sum(item.matched for item in score.critical.values()); running_cost += result.cost_usd or 0.0
            record = VisualCorpusRecord(sample_id=sample_id, model=result.model, latency_ms=latency_ms, input_tokens=result.usage.input_tokens, output_tokens=result.usage.output_tokens, thinking_tokens=result.usage.thinking_tokens, cost_usd=result.cost_usd, character_error_rate=score.character_error_rate, word_error_rate=score.word_error_rate, token_content_recall=score.token_content_recall, token_content_precision=score.token_content_precision, token_content_f1=score.token_content_f1, token_order_preservation=score.token_order_preservation, legal_critical_recall=score.legal_critical_recall, critical_expected_count=expected_critical, critical_matched_count=matched_critical, reference_reliable=health.is_reliable, model_confidence=None, unreadable=False, routed_provider=str((result.provider_metadata or {}).get("routed_provider") or ""))
            records.append(record); details.append({**asdict(record), "object_key": sample["object_key"], "page_index": sample["page_index"]})
        except (ModelProviderError, Exception) as exc: records.append(_record_error(sample_id, f"{type(exc).__name__}: {exc}"))
        if running_cost > MAX_COST_USD: raise RuntimeError(f"cost cap exceeded after page {ordinal}: ${running_cost:.6f} > ${MAX_COST_USD:.2f}")
    ...
    payload = {"schema_version": 2, "manifest_seed": manifest["seed"], "manifest_page_count": len(samples), "model": MODEL, "provider_requested": PROVIDER, "structured_mode": STRUCTURED_MODE, "reasoning": REASONING, "prompt": TRANSCRIPTION_PROMPT, "max_cost_usd": MAX_COST_USD, "summary": summary, "records": details, "failures": [asdict(record) for record in records if record.error], "interpretation": {"reference": "born-digital native PDF text; reliable pages only are scored", "potential_error": "a disagreement with native text, not automatically a proven visual error", "promotion": "benchmark evidence only; no model is promoted by this run alone"}}
