    ...
    try:
        ...
        _event(
            ...
            selected_source_identifiers=[
                candidate.source_identifier for candidate in selected
            ],
        )
        ...
        if not object_store.exists(stored_manifest.object_key):
            raise RuntimeError(
                f"run manifest is not visible after upload: {stored_manifest.object_key}"
            )
        ...
        summary = {
            ...
            "run_manifest_locator": object_store.config.locator_for(
                stored_manifest.object_key
            ),
        }
