...
class PostgresAgentRunLedger:
    """Postgres adapter for observable run state; agents only see StructureRunLedger."""
        ...
        with self.connection.transaction(), self.connection.cursor() as cursor:
            cursor.execute(
                ...
                insert into corpus.agent_runs (
                    ingestion_job_id, artifact_id, parent_run_id, role, round_number,
                    state, provider, model, prompt_sha256, tool_budget, started_at
                )
        ...
        with self.connection.transaction(), self.connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                insert into corpus.agent_run_events (
                    run_id, sequence, occurred_at, event_type, status, tool_name,
                    arguments, result_char_count, result_sha256, result_excerpt,
    ...
    def append_model_turn(self, *, run_id: UUID, event: ModelTurnUsage) -> None:
        with self.connection.transaction(), self.connection.cursor() as cursor:
            ...
            cursor.execute(
                """
                insert into corpus.agent_run_events (
                    run_id, sequence, occurred_at, event_type, status, payload
                )
                values (%s, %s, %s, 'model_turn', 'success', %s::jsonb)
                """,
                (run_id, sequence, event.response_completed_at, _json(event.as_dict())),
            )
        ...
        with self.connection.transaction(), self.connection.cursor() as cursor:
            cursor.execute(
                ...
                insert into corpus.structure_pipeline_runs (
                    ingestion_job_id, artifact_id, idempotency_key, state, round_number
                )
    ...
    def transition_structure_pipeline(
        ...
        *,
        ...
        with self.connection.transaction(), self.connection.cursor() as cursor:
            cursor.execute(
                "select state from corpus.structure_pipeline_runs where id = %s for update",
                (pipeline_run_id,),
            )
            row = cursor.fetchone()
            if row is None:
