...
class RuntimeScope:
    """One agent-stage runtime budget, independent of observability or billing."""
...
def with_runtime_status(system_instructions: str | None, scope: RuntimeScope) -> str | None:
    """Attach the current stage budget to the model-visible system instructions."""
...
class RuntimeSupervisingModel(Model):
    """Model wrapper that owns deadlines, request bounds, and retry policy only."""

    def __init__(self, *, inner: Model, scope: RuntimeScope) -> None:
        self._inner = inner
        self._scope = scope
...
class RuntimeSupervisingModelProvider(ModelProvider):
    """Provider wrapper reusable by tracked and untracked agent executions."""

    def __init__(self, *, inner: ModelProvider) -> None:
        self._inner = inner
        self._scope = RuntimeScope(role="unscoped", round_number=0)
    ...
    def get_model(self, model_name: str | None) -> Model:
        return RuntimeSupervisingModel(
