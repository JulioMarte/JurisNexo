...
def _proposition(cursor: psycopg.Cursor[Any], decision_id: Any, text: str) -> Any:
    cursor.execute(
        """
...
def _evidence(cursor: psycopg.Cursor[Any], decision_id: Any, proposition_id: Any) -> Any:
    cursor.execute(
        """
        INSERT INTO corpus.legal_proposition_evidence (
            proposition_id, source_case_id, evidence_role, extraction_method, exact_excerpt
        ) VALUES (%s, %s, 'supports', 'human_review', 'Fragmento verificado') RETURNING id
        """,
        (proposition_id, decision_id),
    )
    return _one(cursor)
...
def test_judicial_decision_is_physical_identity_and_cases_is_only_compat_view(
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            ...
            WHERE n.nspname = 'corpus' AND relname IN ('judicial_decisions', 'cases')
            ORDER BY relname
            """
        )
        assert cursor.fetchall() == [("cases", "v"), ("judicial_decisions", "r")]
...
def test_dispositions_and_party_roles_use_extensible_concepts(
    ...
    with connection.transaction(force_rollback=True), connection.cursor() as cursor:
        ...
        cursor.execute("SELECT id FROM corpus.disposition_concepts WHERE code = 'cassated'")
        ...
        cursor.execute(
            ...
            ) VALUES (%s, 1, %s, 'CASA la sentencia recurrida', 'human', 'verified')
            """,
            (decision, cassated),
        )
        cursor.execute(
            ...
            WHERE table_schema='corpus' AND table_name='judicial_decision_dispositions'
              AND column_name='disposition_type'
            """
        )
        assert cursor.fetchone() is None

        cursor.execute("INSERT INTO corpus.legal_proceedings (canonical_title) VALUES ('Proceso') RETURNING id")
        proceeding = _one(cursor)
        cursor.execute("INSERT INTO corpus.participants (display_name) VALUES ('Parte') RETURNING id")
        ...
        cursor.execute(
            ...
            INSERT INTO corpus.proceeding_party_roles (
                proceeding_id, participant_id, role_concept_id, party_side, raw_role,
                verification_status, verification_method
            ) VALUES (%s,%s,%s,'claimant','recurrente','verified','primary_text')
            """,
            (proceeding, participant, role_concept),
        )


def test_verified_treatment_requires_source_evidence_and_correct_proposition_membership(
    connection: psycopg.Connection[Any],
) -> None:
    with connection.transaction(force_rollback=True), connection.cursor() as cursor:
        court = _court(cursor)
        ...
        with pytest.raises(psycopg.errors.CheckViolation), connection.transaction():
            cursor.execute(
                ...
                INSERT INTO corpus.legal_treatment_assertions (
                    ...
                    verification_status, verification_method
                ) VALUES (%s,%s,'distinguishes',%s,%s,%s,'verified','human_legal_review')
                """,
        ...
        with pytest.raises(psycopg.errors.CheckViolation), connection.transaction():
            cursor.execute(
                ...
                INSERT INTO corpus.legal_treatment_assertions (
                    source_case_id, target_case_id, treatment_type,
                    issue_proposition_id, source_proposition_id, target_proposition_id,
                    evidence_id, verification_status, verification_method
                ) VALUES (%s,%s,'distinguishes',%s,%s,%s,%s,'verified','human_legal_review')
                """,
                (source, target, source_prop, wrong_prop, target_prop, evidence_id),
            )
        ...
        cursor.execute(
            ...
            """,
            (source, target, source_prop, source_prop, target_prop, evidence_id),
        )


def test_opinion_joiner_must_be_panel_member(connection: psycopg.Connection[Any]) -> None:
    with connection.transaction(force_rollback=True), connection.cursor() as cursor:
        court = _court(cursor)
        decision = _decision(cursor, court)
        officers: list[Any] = []
        for name in ("Jueza autora", "Juez extraño"):
            cursor.execute("INSERT INTO corpus.judicial_officers (display_name) VALUES (%s) RETURNING id", (name,))
            officers.append(_one(cursor))
        author, outsider = officers
        cursor.execute(
            """
            INSERT INTO corpus.decision_panel_members (
                case_id, officer_id, role_raw, panel_role, verification_status, verification_method
        ...
        cursor.execute(
            ...
            INSERT INTO corpus.judicial_opinions (
                case_id, opinion_type, author_officer_id, verification_status, verification_method
            ) VALUES (%s,'majority',%s,'verified','primary_text') RETURNING id
            """,
            (decision, author),
        )
        opinion = _one(cursor)
...
def test_competence_dimensions_are_independent(connection: psycopg.Connection[Any]) -> None:
    with connection.transaction(force_rollback=True), connection.cursor() as cursor:
        court = _court(cursor)
        suffix = uuid4().hex[:8]
        cursor.execute(
            "INSERT INTO corpus.territorial_units (code, name, unit_type) VALUES (%s, 'Santo Domingo', 'province') RETURNING id",
            (f"do_sd_{suffix}",),
        )
        territory = _one(cursor)
        cursor.execute(
            "INSERT INTO corpus.legal_matter_concepts (code, name) VALUES (%s, 'Civil') RETURNING id",
            (f"civil_{suffix}",),
        )
        matter = _one(cursor)
        cursor.execute(
            ...
            INSERT INTO corpus.court_territorial_competences (
                court_id, territorial_unit_id, verification_status, verification_method
        ...
        cursor.execute(
            ...
            INSERT INTO corpus.court_subject_matter_competences (
                court_id, legal_matter_concept_id, verification_status, verification_method
            ) VALUES (%s,%s,'verified','official_registry')
            """,
            (court, matter),
        )
        cursor.execute(
            ...
            INSERT INTO corpus.court_functional_competences (
                court_id, function_type, instance_level, verification_status, verification_method
            ) VALUES (%s,'appellate','second','verified','official_registry')
            """,
            (court,),
        )
        cursor.execute(
            """
            SELECT
              (SELECT count(*) FROM corpus.court_territorial_competences WHERE court_id=%s),
              (SELECT count(*) FROM corpus.court_subject_matter_competences WHERE court_id=%s),
              (SELECT count(*) FROM corpus.court_functional_competences WHERE court_id=%s)
            """,
            (court, court, court),
        )
