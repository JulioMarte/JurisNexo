...
def test_normalization_model_policy_defaults_to_moving_cost_first_aliases(
    ...
    monkeypatch.delenv("JURISNEXO_OPENROUTER_DEEPSEEK_STRUCTURED_MODE", raising=False)
    monkeypatch.delenv("JURISNEXO_OPENROUTER_DEEPSEEK_PROVIDER_ORDER", raising=False)
    monkeypatch.delenv(
        "JURISNEXO_OPENROUTER_DEEPSEEK_ALLOW_PROVIDER_FALLBACKS",
        raising=False,
    )
