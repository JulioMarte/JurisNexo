...
def test_scj_corpus_backfill_discovers_source_scope_and_preserves_no_locator_records() -> None:
    ...
    assert "SCJ_YEAR_DISCOVERY_OUTPUT: ${{ runner.temp }}/scj-year-discovery/scj-year-discovery.json" in workflow
    assert 'path: ${{ runner.temp }}/scj-year-discovery/' in workflow
    assert "SCJ_YEAR_DISCOVERY_FILE: ${{ runner.temp }}/scj-year-discovery/scj-year-discovery.json" in workflow
