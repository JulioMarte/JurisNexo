...
- A read-only Actions diagnostic now reproduces the three failing source/page pairs and emits the native reference, Docling candidate, score and text diff as artifacts. This is an investigation tool, not a replacement for visual/human gold. The diagnostic run has not yet passed; no root cause is asserted until those artifacts are inspected.
