...
- The new shard/coverage/cost reporting and the live workflows switched from `json_object` to the implemented forced-tool mode are **implemented, pending exact-head Actions evidence**. The previous failing `json_object` runs remain evidence of that routed configuration, not proof about all DeepSeek routes.
