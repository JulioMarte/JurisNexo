...
| 4. Planner/durable execution | IMPLEMENTED | deterministic planning, idempotency, durable checkpoints, retry classes, circuit breaker and interrupted-run resume exist. A real worker composition root (`jurisnexo-normalize-run`) now connects canonical acquisition manifests, S3, PostgreSQL, Tika, isolated Docling, deterministic/source-fidelity QA and final reconciliation; live destructive operational-drill evidence remains pending. |
