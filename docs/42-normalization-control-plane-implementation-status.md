...
- After the full-inventory failure, the ordinary PR suite must evaluate **all persisted page records with the current benchmark identity**, including failures discovered by a larger dispatch. It also emits `current_selection_report` for the 12-document PR selection. This prevents a later short run from hiding known failures. Report schema v4 changes this gate scope without invalidating v3 page evaluations; the benchmark identity remains pinned to the same normalizer, scorer, selector and Docling version. This revised cumulative gate is implemented, pending Actions proof.
