...
- The first forced-tool run pinned to the `deepseek` provider failed before inference: OpenRouter routing reported no endpoint compatible with both the tool request and the no-fallback provider constraint (runs `35939079364` and `35939079358`). With fallback routing, the bounded text smoke `35939854334` passed: DeepSeek V4.1 Flash was served by Wafer, three model calls completed, and observed provider cost was US$0.000935988. The visual smoke `35939854252` still failed: AtlasCloud and StreamLake returned prose instead of tool-call arguments. A separate GPT-4.1 Mini visual challenger (`35940259389`, Azure) returned structured responses for two calls at US$0.002544, but failed the semantic test: corruption detection recall 0 and false correction rate 1. The benchmark itself had compared a full-page image with only the first 2,500 native-text characters, so its positive case was not a valid whole-page gold assertion. The revised lane sends the complete native page text and labels that reference unverified against the image; its semantic result is pending. Neither VLM is promoted. The earlier `json_object` failures remain evidence of that routed configuration, not proof about all DeepSeek routes.
