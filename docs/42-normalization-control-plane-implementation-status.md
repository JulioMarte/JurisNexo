...
- A read-only `selector_audit` dispatch will inspect all pages of the 12 excluded volumes from the frozen full-inventory run. It reports total pages, native-text counts, qualifying adjudicative-page counts and the first qualifying page index. This distinguishes an overly short 120-page window from a genuinely different layout or poor native layer. It does not itself validate normalization quality. The audit is implemented, pending Actions evidence.
