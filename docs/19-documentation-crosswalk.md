...
- `39-document-normalization-and-derived-artifacts.md` — current contract separating official source artifacts from immutable technical derivatives and normalization-run provenance;
- `40-document-normalization-pipeline-v2-handoff.md` — historical/partially superseded V2 implementation rationale retained for detailed quality/provenance decisions carried forward by V3;
- `41-universal-legal-document-normalization-control-plane-v3-handoff.md` — **current normalization implementation handoff**, defining the source-agnostic JurisNexo control plane over Docling/Tika and replaceable OCR/JEV/VLM capabilities;
