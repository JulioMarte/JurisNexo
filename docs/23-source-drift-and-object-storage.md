...
Large production backfills keep an append-only local recovery journal during each shard. The journal is flushed and fsynced after every resolved source observation. Its compact latest-state view is also mirrored after every observation under the reserved mutable namespace `_checkpoints/<source>/<scope>/<snapshot-or-run-identity>/...`, independently from the immutable final run-manifest namespace. Inventory-driven jobs scope checkpoints by the exact inventory SHA-256 (the certified inventory SHA for SCJ 1994+); Principales scopes by explicit ingestion identity so a future discovery run cannot silently inherit stale observations. GitHub Actions still uploads the local evidence with `if: always()` as a second recovery surface. Stored records contain the source identifier/URL, SHA-256, object key, byte count and detected format. Unavailable and item-failure observations are recorded separately.

The checkpoint namespace is operational state, not canonical legal evidence and not an inter-system completion contract. A checkpoint may be overwritten by the next observation for the same snapshot/run shard, but it must never be reused across a different checkpoint identity. The mirror stores both a SHA-256 of the checkpoint payload and the checkpoint identity in object metadata; restore recomputes the digest and validates the identity before any record is trusted. Missing, corrupt or mismatched checkpoint metadata fails closed. The immutable run manifest remains the closure/handoff record. This deliberate split means a runner hard-kill no longer depends solely on the end-of-job artifact upload. There is still an unavoidable non-atomic window between committing a content-addressed source object and persisting the next checkpoint; reconciliation detects any object left behind by that window.
...
An interrupted large backfill writes recovery evidence and exits non-zero with `RUN_INTERRUPTED`, the cause, the interruption point and `resumable: true`. This applies consistently to checkpoint restore, acquisition/checkpoint persistence, manifest commit and normal `SIGTERM`/`SIGINT` shutdown. Expected infrastructure interruptions use a controlled process exit rather than manufacturing a traceback as though application code had crashed. The job does **not** attempt to publish a successful final object-storage run manifest when storage itself is the failing dependency. The final immutable run manifest remains a closure record and is committed only after the job reaches a controlled end.
...
### Recovery-provider dependency

The current remote checkpoint mirror intentionally uses the same configured S3-compatible provider as source artifacts. This protects recovery from ephemeral runner loss but is not a physically independent durability domain. If the provider refuses both the legal-object write and the following checkpoint write, the latest remote checkpoint can lag the object store by one observation. JurisNexo accepts this bounded MVP tradeoff because source objects are content-addressed and final reconciliation fails closed on unreferenced objects; recovery can re-download the affected official URL, recompute its SHA-256 and adopt the already-present bytes without duplicating canonical storage. A second independent checkpoint provider may be added later if operational evidence justifies the extra system.
