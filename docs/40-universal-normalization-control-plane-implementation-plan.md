...
- DeepSeek is the economical general-reasoning challenger/verifier. Prefer the cheapest currently suitable DeepSeek tier exposed by OpenRouter (currently the V4 Flash family), even when it is slower. Latency is secondary to cost for offline normalization benchmarks unless a later measured SLO says otherwise.
...
Current routing intent (2026-09-20): OpenRouter lists JEV as a low-cost structured decision model and exposes moving JEV and DeepSeek family aliases. DeepSeek V4 Flash is the cost-oriented family; JurisNexo deliberately accepts slower completion when that reduces offline normalization/evaluation cost. This is policy, not a permanent model-version pin: capability and benchmark evidence can override the cheapest option when the cheap tier cannot satisfy the task contract.
