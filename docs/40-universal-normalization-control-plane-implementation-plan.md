...
Model access policy for normalization/evaluation:
- OpenRouter is the configured gateway for hosted JEV and DeepSeek calls; provider credentials never enter source control.
- CI/manual provider tests consume `OPENROUTER_API_KEY` from the execution environment/GitHub secret of the same name.
- JEV uses the OpenRouter JEV family endpoint behind `TextQualityJudge`; prefer the moving `Jev Latest` alias for exploratory/shadow evaluation, while every persisted observation records the concrete model/provider identity returned by the call when available.
- DeepSeek is the general-reasoning challenger/verifier. The current JurisNexo benchmark/runtime policy selects `deepseek/deepseek-v4.1-flash` with reasoning effort `high`. `xhigh` (the max-equivalent OpenRouter effort for this family) is reserved for explicit comparison benchmarks until evidence justifies promoting it.
- Keep model selection configurable rather than coupling the architecture to a provider SKU. Every benchmark persists requested/effective model identity, requested reasoning effort, provider/routing metadata, input/output/reasoning-token usage, latency, cost when available, and configuration. Future `high` vs `xhigh` comparisons must use the same cases and decision contract.
- No provider call is legal ground truth. Primary-source bytes/evidence and deterministic provenance remain authoritative.
- External model use remains subject to the provider/privacy gate before private or sensitive corpora may be sent.
...
## Hosted-model test configuration

The repository contract for live provider tests is `OPENROUTER_API_KEY`. Tests that require network/provider access must be explicitly selected and must skip with a clear reason when the key is absent; ordinary unit, architecture and database CI must remain deterministic and must not spend model credits. GitHub Actions should inject the key from an encrypted repository/environment secret only into the dedicated live benchmark job. Never print the key, request headers, or secret-bearing configuration in logs/artifacts.

Current routing intent (2026-09-21): JEV is a structured decisions model and must use the OpenRouter decisions API, not chat completions. DeepSeek `deepseek/deepseek-v4.1-flash` is the configured general-reasoning challenger, defaulting to reasoning effort `high`. `xhigh` is an experimental comparison mode, not the default. Capability/benchmark evidence may change this policy later, but any change must retain comparable token/cost/latency telemetry.
